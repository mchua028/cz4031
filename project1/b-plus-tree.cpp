...
class BPTree
{
    ...
    void removeInternal(keys_struct x, Node* cursor, Node* child)
    {
        ...
        for(int i = pos; i < cursor->size && i+1 < MAX; i++)
        {
        ...
        else
        {
            ...
            while(cursor->IS_LEAF == false)
            {
                for(int i = 0; i < cursor->size; i++)
                {
                    ...
                    if(i < MAX && x.key_value < cursor->key[i].key_value)
                    {
                        cursor = cursor->ptr[i];
                        break;
                    }
            ...
            if(leftSibling >= 0 && leftSibling < MAX)
            {
                Node *leftNode = parent->ptr[leftSibling];
                //check if it is possible to transfer
            ...
            if(rightSibling <= parent->size && rightSibling >= 0)//check if right sibling exist
            {
                Node *rightNode = parent->ptr[rightSibling];
                //check if it is possible to transfer
            ...
            if(leftSibling >= 0 && leftSibling < MAX+1)//if left sibling exist
            {
                Node* leftNode = parent->ptr[leftSibling];
